...
/**
 * V4 Landing Page — exact 1:1 port of linki---stand-out zip file design.
 * Gradient: blue-to-cyan (#2563eb → #06b6d4), matching the zip's text-gradient.
 * CTAs link to /v4/audit (the real intake form with the zip's UI).
 */
...
/* ── GRADIENT UTIL ─────────────────────────────────────────────────── */
// The zip uses linear-gradient(to right, #2563eb, #06b6d4)
  ...
  const btn = (
    <motion.button
      ...
      className={[
        ...
        variantCls, sizeCls, className, disabled ? "opacity-50 cursor-not-allowed" : "",
      ].join(" ")}
    >
...
function MouseSpotlight() {
  ...
  return (
    <motion.div
      ...
      style={{
        background: useMotionTemplate`radial-gradient(600px circle at ${mx}px ${my}px, rgba(37,99,235,0.08), transparent 40%)`,
      }}
    />
  );
}
        ...
        >
          <Link href="/" className="flex items-center">
            <AnimatePresence mode="wait">
              {!scrolled ? (
                <motion.img key="full" src="/logos/linkifyme-text.png" alt="LinkifyMe"
                ...
                <motion.img key="icon" src="/logos/linkifyme-icon.png" alt="Li"
          ...
          <div className="hidden md:block">
            <Btn variant={scrolled ? "primary" : "outline"} size="sm"
              className={!scrolled ? "text-rich-black border-black/10" : ""}
              href="/v4/audit">
              Get Started
            </Btn>
          </div>
        ...
        {mobileOpen && (
          ...
          >
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
              ...
              <Link href="/v4/audit" onClick={() => setMobileOpen(false)}>
    ...
    <section ref={ref} className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <motion.div style={{ y: yText, opacity: opacityText }}>
          ...
          >
            Command <br />
            <span style={GRADIENT_TEXT_STYLE}>Attention.</span>
          </motion.h1>
          ...
          >
            <Btn size="lg" className="w-full sm:w-auto min-w-[200px] h-14 text-lg group" href="/v4/audit">
              Audit Profile
              <ArrowRight className="w-5 h-5 transition-transform group-hover:translate-x-1" />
            </Btn>
            <Btn variant="outline" size="lg" className="w-full sm:w-auto h-14 text-lg" href="#demo">
              View Demo
            </Btn>
          </motion.div>
        </motion.div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          <h2 className="text-4xl md:text-5xl font-bold mb-6 text-rich-black tracking-tight">
            The <span style={GRADIENT_TEXT_STYLE}>Shift</span>
          </h2>
          <p className="text-gray-500 text-lg max-w-2xl mx-auto">Stop competing in the noise. Ascend to the signal.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          >
            ...
            <ul className="space-y-6 text-gray-400">
              {['Generic "Open to work" frames', "Keyword-stuffed, unreadable bios", "Zero social proof or metrics", "Passive waiting for recruiters"].map((item, i) => (
                <li key={i} className="flex items-start">
                  <span className="w-1.5 h-1.5 rounded-full bg-gray-300 mt-2 mr-3 shrink-0" />{item}
                </li>
              ))}
            </ul>
          </motion.div>
          ...
          >
            <div className="absolute inset-0 opacity-[0.03]" style={{ background: "linear-gradient(to right, #2563eb, #06b6d4)" }} />
            ...
            <ul className="space-y-6 text-gray-700 relative z-10">
              {["Psychologically engineered hooks", "Metric-driven achievement stack", "Automated inbound lead generation", "Top 1% SSI Score Authority"].map((item, i) => (
                <li key={i} className="flex items-start">
                  <span className="w-1.5 h-1.5 rounded-full bg-brand mt-2 mr-3 shrink-0" />{item}
                </li>
              ))}
            </ul>
          </motion.div>
...
function Timeline() {
  ...
  const steps = [
    ...
    { id: 3, icon: FileText, title: "Receive Summary", description: "Get your personalized action plan instantly.", color: "from-cyan-500 to-teal-400" },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            How It{" "}
            <span style={{ background: "linear-gradient(to right, #2563eb, #06b6d4)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
              Works
            </span>
          </motion.h2>
          <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            transition={{ delay: 0.1 }} className="text-gray-500 max-w-2xl mx-auto">
        ...
        <div className="relative">
          ...
          <motion.div className="hidden md:block absolute top-8 left-0 h-1 z-0 rounded-full origin-left"
            style={{ scaleX: scrollYProgress, width: "100%", background: "linear-gradient(to right, #2563eb, #06b6d4)" }} />
          <div className="md:hidden absolute top-0 left-8 bottom-0 w-1 bg-gray-100 z-0 rounded-full" />
          <motion.div className="md:hidden absolute top-0 left-8 w-1 z-0 rounded-full origin-top"
            style={{ scaleY: scrollYProgress, height: "100%", background: "linear-gradient(to bottom, #2563eb, #06b6d4)" }} />
            ...
            {steps.map((step, index) => (
              <motion.div key={step.id}
                ...
                <div className="flex items-center justify-center w-16 h-16 rounded-2xl bg-white border border-gray-100 shadow-lg mb-6 group-hover:scale-110 transition-transform duration-300 relative z-10">
                  ...
                  <step.icon className="w-8 h-8 text-gray-700 group-hover:text-brand transition-colors" />
    ...
    <section className="py-32 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="relative h-[400px] flex items-center justify-center">
          ...
          <div className="absolute border border-black/5 rounded-full w-[500px] h-[500px] animate-slow-spin opacity-30"
          ...
          {logos.map((logo, i) => {
            ...
            return (
              <motion.div key={logo}
                ...
                style={{ x, y }}
                animate={{ x: [x, Math.cos(angle + Math.PI * 2) * r], y: [y, Math.sin(angle + Math.PI * 2) * r] }}
...
/* ── FEATURES ───────────────────────────────────────────────────────── */
const FEATURES = [
  { title: "Instant Velocity", description: "Analysis results delivered in under 200ms using edge caching.", icon: Zap, colSpan: "md:col-span-2" },
  { title: "Global Reach", description: "Benchmark your profile against top 1% globally.", icon: Globe, colSpan: "md:col-span-1" },
  { title: "AI Precision", description: "Neural networks trained on 10M+ profiles for exact recommendations.", icon: Cpu, colSpan: "md:col-span-1" },
  { title: "Growth Metrics", description: "Track your SSI score improvements over time.", icon: TrendingUp, colSpan: "md:col-span-2" },
];
...
function FeatureCard({ f, i }: { f: typeof FEATURES[0]; i: number }) {
  ...
  return (
    <motion.div ref={ref}
      ...
      className={`group relative overflow-hidden rounded-3xl border border-black/5 bg-white p-8 ${f.colSpan} hover:border-brand/30 transition-shadow shadow-sm hover:shadow-lg`}
    >
      <div className="pointer-events-none absolute -inset-px opacity-0 transition duration-300 group-hover:opacity-100">
        <motion.div className="absolute inset-0 opacity-30"
          style={{
            background: "linear-gradient(to right, rgba(30,58,138,0.1), rgba(37,99,235,0.1))",
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        ...
        <div>
          <h3 className="mb-2 text-xl font-bold text-rich-black">{f.title}</h3>
    ...
    <section id="features" className="py-32 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <h2 className="text-4xl md:text-5xl font-bold mb-6 text-rich-black tracking-tight">
            Engineered for <span style={GRADIENT_TEXT_STYLE}>Domination</span>
          </h2>
  ...
  return (
    <section id="stories" className="py-24 relative overflow-hidden">
      ...
      <div className="flex overflow-hidden">
        <motion.div className="flex gap-8 px-4"
          animate={{ x: ["0%", "-50%"] }}
          transition={{ repeat: Infinity, duration: 40, ease: "linear" }}>
          {[...STORIES, ...STORIES].map((s, i) => (
            <div key={i} className="flex-shrink-0 w-[400px] p-8 rounded-2xl bg-white border border-black/5 shadow-sm hover:shadow-md transition-shadow group">
        ...
        <div className="relative aspect-video w-full bg-white rounded-[2rem] shadow-2xl overflow-hidden border-4 border-white group cursor-pointer">
          <div className="absolute inset-0 bg-slate-900 flex items-center justify-center">
            ...
            <img src="https://images.unsplash.com/photo-1551434678-e076c223a692?auto=format&fit=crop&w=1200&q=80"
              alt="Dashboard preview" className="opacity-30 w-full h-full object-cover" />
...
/* ── FAQ ────────────────────────────────────────────────────────────── */
const FAQS = [
  { q: "Is this cheating?", a: "No. It's engineering. We use the same algorithms LinkedIn uses to rank profiles, but in reverse to optimize yours." },
  { q: "How long does the audit take?", a: "Less than 200 milliseconds. Our edge computing infrastructure analyzes 50+ data points instantly." },
  { q: "Will my current employer know?", a: "We operate in stealth mode. No notifications are sent. Your optimization process is completely private." },
  { q: "What is the SSI Score?", a: "Social Selling Index. It's the hidden metric LinkedIn uses to decide who sees your content. We help you max it out." },
];
          ...
          {FAQS.map((faq, i) => (
            ...
            >
              <button className="w-full flex items-center justify-between p-6 text-left group"
                ...
                {open === i ? <Minus className="w-5 h-5 text-brand shrink-0" /> : <Plus className="w-5 h-5 text-gray-400 group-hover:text-rich-black transition-colors shrink-0" />}
              </button>
              <AnimatePresence>
                {open === i && (
    ...
    <section className="py-40 relative flex items-center justify-center">
      <div className="relative z-10 text-center max-w-4xl px-4">
        <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          Don&apos;t vanish. <br />
          <span style={GRADIENT_TEXT_STYLE}>Stand out.</span>
        </motion.h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          <div className="col-span-1 md:col-span-2">
            <span className="text-2xl font-bold tracking-tighter text-white mb-4 block">
              linki<span style={{ color: "#4361ee" }}>.</span>
            </span>
...
export default function V4Page() {
  return (
    <div className="min-h-screen bg-white text-rich-black font-sans selection:bg-brand selection:text-white overflow-x-hidden relative">
      {/* Global atmosphere — exact from zip */}
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-[-15%] right-[-15%] w-[45vw] h-[45vw] bg-blue-600/15 rounded-full blur-[120px] mix-blend-multiply animate-slow-spin"
